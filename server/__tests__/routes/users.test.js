...
describe('users route: not logged in', () => {
  ...
  test("GET '/:id/profile' -- should fail", async () => {
    let { status, body } = await anon.get(`${userApi}/${newUserId}/profile`);
...
describe('users route: user', () => {
  ...
  test("GET '/:id/profile' -- should fail (not owner)", async () => {
    let { status, body } = await user.get(`${userApi}/${adminId}/profile`);
    expect(body.type).toEqual('NotAuthorized');
    expect(status).toEqual(403);
  });
