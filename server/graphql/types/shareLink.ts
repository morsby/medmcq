...
export const resolvers: Resolvers = {
  Query: {
    shareLink: async (_args, { shareId }) => {
      const shareObjs = await ShareLink.query().where('shareId', '=', shareId).select('questionId');
  ...
  Mutation: {
    createShareLink: async (_args, { questionIds }, ctx) => {
      ...
      const randomId = crypto.randomBytes(8).join('').substring(0, 10);

      // HVIS det utænkeligt skulle ske, at ID'et er det samme
      const alreadyExists = await ShareLink.query().where('shareId', '=', randomId).first();
      ...
      const shareLink = await ShareLink.query().insertGraphAndFetch(links).first();
