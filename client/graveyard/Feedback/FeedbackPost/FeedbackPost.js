            ...
            <Form
              ...
              validate={(values) => {
                ...
                if (!values.title || values.title.length < 3) {
                  errors.title = translate('feedbackPost.errs.title_too_short', { min: 3 });
                }

                if (!values.text || values.text.length < 10) {
                  errors.text = translate('feedbackPost.errs.text_too_short', { min: 10 });
                }
                return errors;
              }}
              render={({ handleSubmit, submitting, values, form }) => (
                <form onSubmit={handleSubmit} className="ui form">
                  ...
                  <h4>{translate('feedbackPost.preview.header')}</h4>
                  <FeedbackSingleContent
                    feedback={{
                      title: values.title || translate('feedbackPost.form_fields.title'),
                      date: new Date(),
                      text: values.text || translate('feedbackPost.form_fields.suggestion')
                    }}
                  />
                </form>
              )}
